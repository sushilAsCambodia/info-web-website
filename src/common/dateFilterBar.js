...
export default function DateFilterBar(props) {
  ...
  return (
    <Grid container  justifyContent="center" pb={1} className='StickyDate' >
    <Grid item xs={12} md={12} container flexWrap="nowrap" sx={{ overflow:"auto"}}  >
      {utils.dataRangeLastNext(3).map((item, index) => {
        return (
          ...
          >
            <Typography
              sx={{ fontWeight: "bold", fontSize: {xs:"12px",md:"13px"} }}
              px={0.5}
            >
              {item.day}
            </Typography>
            <Typography px={0.5} sx={{ fontSize: {xs:"12px",md:"13px"} }}>
              {item.DateMonth}
            </Typography>
          </Grid>
        );
