...
return InfoDetails && InfoDetails.liveTimelines && InfoDetails.liveTimelines.length>0 && InfoDetails.liveTimelines.map((item, index) => {
  const eventType=item && item.eventType
  if(item && item.eventType!=11)
  return (
    <div style={{width:'100vw'}} key={index}>
       <div className="timeline">
        <div style={{"--title":`'${item.minute+'`'}'`}} className={`${item && item.teamId===homeTeamId ? "left":"right" } container`}>
          <div className="content">
            <div className="match-icon" style={{width:'35px'}}>{manageEvent(eventType)}</div>
            {/* <img src="/assets/Logo/1200px-Missed_penalty_icon.svg" /> */}
            <h4>{item.playerName!=null?item.playerName:"__"}</h4>
            <p style={{fontSize:'10px'}}>
            {item.relatedPlayerName!=null?item.relatedPlayerName:" "}
            </p> 
          </div>
        </div>
      </div>
