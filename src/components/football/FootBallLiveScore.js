...
export default function FootBallLiveScore({footballFavoritList,lang_id,loadings,
    ...
    <>
        ...
        <Grid item xs={12}>
          <TableContainer component={Paper}>
            <Table sx={{ minWidth: 700 }} className={classes.table} id="tablehover"  aria-label="customized table">
              <TableHead>
                <TableRow>
                  <StyledHeaderCell width="200px">{langKey && langKey.competition}</StyledHeaderCell>
      ...
      >
        <CircularProgress color="inherit" />
              ...
              <TableBody>
                {footballLiveScoreList && footballLiveScoreList.length>0 ? footballLiveScoreList.map((item, index) => {
                     let stage=item && item.match_schedule && item.match_schedule.stage
                     let font_color
                 let background_color                  
                 if(item && item?.match_schedule?.competition && item?.match_schedule?.competition?.background_colour===null){
                   background_color="#e60039"
                 } else {
                   background_color=item?.match_schedule?.competition?.background_colour
                 }
                 if(item && item?.match_schedule?.competition && item?.match_schedule?.competition?.font_colour===null){
                   font_color="#ffffff"
                 } else {
                      ...
                      <StyledTableCell align="left"  style={{height:"45px!important",color:font_color, background:background_color }}>
                          ...
                          >
                            ...
                            <Typography mx={1} fontSize={'13px'}>                           
                              {lang_id==1?item?.match_schedule?.competition?.nameEn:lang_id==2?item?.match_schedule?.competition?.name:item?.match_schedule?.competition?.nameEn}
                            </Typography>
                          </Grid>
                        </StyledTableCell >
                        
                        <StyledTableCell align="center">
                        {lang_id==1?stage && stage.match(regex):lang_id==3?stage && stage.match(regex):stage}
                        {/* {stage && stage.match(regex)} */}
                        </StyledTableCell>
                        <StyledTableCell align="center">
                        {moment(item?.match_schedule?.startTime).format('HH:mm')}
                        </StyledTableCell>
                        <StyledTableCell align="center">
                        {item.elapsed}
                        </StyledTableCell>
